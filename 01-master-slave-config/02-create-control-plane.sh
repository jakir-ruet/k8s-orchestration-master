...
# range doesn't overlap with other network's in our infrastructure.
vi calico.yaml

# You can now use kubeadm init to bootstrap the cluster, notice the format for version is shorter than the apt package's.
sudo kubeadm init --kubernetes-version v1.32.0 #--pod-network-cidr w.z.y.z/16
...
# Get a list of our current nodes, just the Control Plane Node...should have a status of Ready.
kubectl get nodes
