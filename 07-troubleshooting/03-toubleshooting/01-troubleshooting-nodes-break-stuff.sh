...
ssh $USER@c1-wnode1 -t 'sudo systemctl stop kubelet.service'
ssh $USER@c1-wnode1 -t 'sudo systemctl disable kubelet.service'

#  Worker Node - inaccessible config.yaml
ssh $USER@c1-wnode2 -t 'sudo mv /var/lib/kubelet/config.yaml /var/lib/kubelet/config.yml'
ssh $USER@c1-wnode2 -t 'sudo systemctl restart kubelet.service'

#  Worker Node - misconfigured systemd unit
ssh $USER@c1-wnode3 -t 'sudo sed -i ''s/config.yaml/config.yml/'' /etc/systemd/system/kubelet.service.d/10-kubeadm.conf'
ssh $USER@c1-wnode3 -t 'sudo systemctl daemon-reload'
ssh $USER@c1-wnode3 -t 'sudo systemctl restart kubelet.service'
